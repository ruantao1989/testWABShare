  ...
  return declare([_WidgetBase], {
    ...
    getClientStatisticsData: function(options, forServer) {
      ...
      if (mode === 'feature') {
        ...
        //seriesStyle.useLayerSymbology/*optional*/}
    ...
    _createChartSeries: function(options, data) {
      ...
      var mode = chartConfig.mode;
      if (mode === 'feature') {
        return this._getFeatureModeChartOptionsByStatisticsInfo(options, data);
      } else if (mode === 'category') {
    ...
    _getFeatureModeChartOptionsByStatisticsInfo: function(options, data) {
      ...
      data.forEach(lang.hitch(this, function(item) {
        ...
        for (var i = 0; i < item.valueFields.length; i++) {
          ...
          var dataItem = {
            dataFeatures: item.dataFeatures,
            value: value,
            name: label,
            _originName: _originLabel
          };
          chartOptions.series[i].data.push(dataItem);
        }
      }));
      var series = this._assigneeSettingColor(chartConfig, chartOptions.series, featureLayerForChartSymbologyChart);
      this._removeUselessSeriesDataFeatures(series);
    ...
    _assigneeSettingColor: function(chartConfig, series, featureLayerForChartSymbologyChart) {
      ...
      if (useLayerSymbology) {
        series.forEach(function(serie) {
          ...
          if (data && data.length) {
            data.forEach(function(dataItem) {
              var features = dataItem.dataFeatures;
        ...
        series = this._assigneeColorsToSeries(chartConfig, series);
      }
      return series;
    },

    // set one color to all series's data item
    _setColorToAllSerieDataItem: function(series, otherColor) {
      series.forEach(function(serie) {
        var data = serie.data;
        if (data && data.length) {
          data.forEach(function(dataItem) {
            this._setColorToDataItem(dataItem, otherColor);
          }.bind(this));
        }
      }.bind(this));
    },

    _removeUselessSeriesDataFeatures: function(series) {
      series.forEach(function(serie) {
        var data = serie.data;
        if (data && data.length) {
          data.forEach(function(dataItem) {
            if (typeof dataItem.dataFeatures !== 'undefined') {
              delete dataItem.dataFeatures;
            }
          });
        }
      });
    ...
    _setColorToDataItem: function(item, color) {
    ...
    _setColorToSerie: function(serie, color) {
    ...
    _assigneeColorsToSeries: function(chartConfig, series) {
      var seriesStyle = chartConfig.seriesStyle;
      if (!seriesStyle || !seriesStyle.styles || !seriesStyle.styles[0]) {
        return series;
      }
